...
impl<'ast, 'reporter, R: Reporter> MutVisitor<'ast, Option<ResolvedType>> for TypeChecker<'reporter, R> {
    ...
    fn visit_func_decl(&mut self, func: &'ast mut FunctionDecl) -> Option<ResolvedType> {
        ...
        for arg in &mut func.args {
            let Some(arg_ty) = resolve_type(arg.ty.span.text()) else { panic!("Could not resolve type of function argument {}", arg.name.text()); };
            arg.ty.resolved = Some(arg_ty.clone());
            self.add_variable(arg.name.text().to_owned(), arg_ty);
        }
        ...
        match self.visit_expr(&mut func.body) {
            ...
            Some(ResolvedType::Empty) => {
                ...
                if !self.current_function.as_ref().unwrap().is_explicit_return_guaranteed() {
    ...
    fn visit_return(&mut self, expr: &'ast mut Return) -> Option<ResolvedType> {
        ...
        self.current_function.as_mut().unwrap().set_explicit_return_guaranteed();
